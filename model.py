...
class BoschModel():
    ...
    def __init__(self, n_part_types):
    ...
    def build_model_data(self,subset=-1):
        ...
        for i,part in enumerate(part_data[0:subset]):
            if i % 10000 == 0:
        ...
        with open('data/' + str(self.n_part_types) +  '_part_data.json', 'w') as outfile:
    ...
    def save_model(self):
        with open('models/' + str(self.n_part_types) +  '_model.pkl', 'wb') as outfile:
    ...
    def build_submission(self):
        ...
        for numeric, categorical, date in reader:
            ...
            for index, part in date_chunk.iterrows():
                ...
                feature_matrix = timestamp_matrix + value_matrix
                clf = self.models[part_type]
                pred = clf.predict_proba(feature_matrix)
                print (pred)
                with open('submission/' + str(self.n_part_types) + "_submission", 'w') as f:
                    writer = csv.writer(f)
                ...
                if pred >= self.mcc_cutoffs[part_type]:
                    ...
                    response=0
                with open('submission/' + str(self.n_part_types) + "_submission", 'a') as f:
                    writer = csv.writer(f)
...
if  __name__ == "__main__":
    test_model = BoschModel(3)
    test_model.classify_part_types()
    test_model.build_model_data(10000)
